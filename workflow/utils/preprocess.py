...
def label_fasta_fragments(sequences, label):
    ...
    label - type of label (bacteria, virus, eucaryotic)
    Output:
    labeled_fragments - list with labeled SeqRecord sequences
    """
    assert label in ["virus", "eucaryotic", "bacteria"]
    labeled_fragments = []
    for sequence in sequences:
...
#     encoded = np.concatenate(([one_hot_encode.remote(s) for s in it]))
#     it = chunks(frags_rc, int(len(frags_rc) / n_cpus + 1))
#     encoded_rc = np.concatenate(([one_hot_encode.remote(s) for s in it]))
...
def prepare_ds_fragmenting(in_seq, label, label_int, fragment_length, sl_wind_step, max_gap=0.05, n_cpus=1):
    ...
    # Encoding fragments
    encoded = np.concatenate([one_hot_encode(s) for s in chunks(frags, int(len(frags) / n_cpus + 1))])
    encoded_rc = np.concatenate([one_hot_encode(s) for s in chunks(frags_rc, int(len(frags_rc) / n_cpus + 1))])
...
#     # ray.init(num_cpus=n_cpus, num_gpus=0, include_dashboard=False)
#     # generating eucaryotic fragments and labels
#     seqs_list = prepare_seq_lists(in_seqs, n_frags)
#     it = chunks(seqs_list, int(len(seqs_list) / n_cpus + 1))
#     frs_ = [sample_fragments.remote(s, fragment_length, random_seed, limit=limit, max_gap=0.05) for s in it]
...
def prepare_ds_sampling(in_seqs, fragment_length, n_frags, label, label_int, random_seed, n_cpus=1, limit=100):
    ...
    # Sample fragments directly
    frs_ = [sample_fragments(s, fragment_length, random_seed, limit=limit, max_gap=0.05) for s in it]
    ...
    # Encode fragments
    it = chunks(frags, int(len(frags) / n_cpus + 1))
    encoded = np.concatenate([one_hot_encode(s) for s in it])
    
    it = chunks(frags_rc, int(len(frags_rc) / n_cpus + 1))
    encoded_rc = np.concatenate([one_hot_encode(s) for s in it])
